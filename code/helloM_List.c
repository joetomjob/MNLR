...
int _get_MACTest(struct addr_tuple *myAddr, int numTierAddr) {
	...
	while (1) { //this is an infinite loop
		...
		if (!endNode) { // only does when node is an edge node.
					...
					if (packetFwdStatus == 0) {
						...
						if ((strlen(fwdTierAddr) == strlen(tierAddress))
							...
							if (isFWDFieldsSet() == 0) {
								...
								if(enableLogFiles){
									fprintf(fptr,"TEST: Forwarding IP Packet as MPLR Data Packet (Encapsulation) \n");
...
//The argument comes as a single string. we need to split and save them to following variables
	...
	if(enableLogFiles)
		fprintf(fptr," Writing logs to file ... ");
	fflush(fptr); 
	...
	if (argc > 1) { //if the number of arguments is greater than 1, we will go forward
		...
		while (argc > 0) { //the loop will continue until the number of arguments is greater than 1. 
			...
			opt = argv[0]; //assign "-T" to opt

			// Checking for -T
			if (strcmp(opt, "-T") == 0) { //compare opt and "-T"
				...
				do {
					...
					if (next[0] == '-') { //if the first element of next is "-", we exit the do while loop. 
						break; //break from do while llop
					}

					/* For Default tier address */
					if (tierSpecial == false) { //initally this is set as false
